...
object Day6 extends Day(6) {
  ...
  def processDeclarations(
      lines: List[String],
      currentSet: Seq[Set[Char]] = Seq.empty,
      processedGroups: Seq[Seq[Set[Char]]] = Seq.empty
  ): Seq[Seq[Set[Char]]] = {
    lines match {
  ...
  def sumOfAllAnsweredQuestions(lines: List[String]): Int = {
    processDeclarations(lines)
      .map(answers => answers.foldLeft(answers.headOption.getOrElse(Set.empty))((a, b) => a.intersect(b)).size)
      .sum
  }
